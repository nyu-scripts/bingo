...
### host.html
`prng.js` -> `theme.js` -> `game.js` -> `card.js` -> `win.js` -> `host.js`
...
| `js/host.js` | Host page controller — game creation, draw loop, pattern picker, custom grid builder, bingo verification by card seed |
