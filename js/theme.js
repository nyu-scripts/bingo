// Theme registry and placeholder SVG generation

const PALETTE = [
  "#e74c3c", "#e67e22", "#f1c40f", "#2ecc71", "#1abc9c",
  "#3498db", "#9b59b6", "#e84393", "#00cec9", "#6c5ce7",
  "#fd79a8", "#00b894", "#fdcb6e", "#74b9ff", "#a29bfe",
  "#fab1a0", "#55efc4", "#81ecec", "#dfe6e9", "#ffeaa7",
];

// Manifest + inline fallback items for built-in themes.
// When served via HTTP, loadTheme() fetches themes/<id>.json instead.
const THEMES = [
  { id: "default", title: "Numbers", items: [] },
  { id: "emojis", title: "Emojis", items: [] },
  { id: "biscuitneopets", title: "Biscuit Neopets", items: [] },
  { id: "candyneopets", title: "Candy Neopets", items: [] },
  { id: "greenneopets", title: "Green Neopets", items: [] },
  { id: "woodlandneopets", title: "Woodland Neopets", items: [] },
  { id: "paintbrushes", title: "Paint Brushes", items: [] },
  { id: "neggs", title: "Neggs", items: [] },
  { id: "cybunnies", title: "Cybunnies", items: [] },
];

function getThemeList() {
  return THEMES.map(function(t) { return { id: t.id, title: t.title }; });
}

function placeholderSvg(name, index) {
  // Emojis have intrinsic colors and ignore SVG fill, so use a neutral dark bg
  const isEmoji = /\p{Emoji_Presentation}|\p{Extended_Pictographic}/u.test(name);
  const bg = isEmoji ? "#2d2d3d" : PALETTE[index % PALETTE.length];
  const fill = luma(bg) > 0.5 ? "#222" : "#fff";
  const label = escapeXml(name);

  // Word-wrap into lines of ~12 chars
  const maxChars = 12;
  const words = label.split(/\s+/);
  const lines = [];
  let current = "";
  for (let i = 0; i < words.length; i++) {
    if (current && (current.length + 1 + words[i].length) > maxChars) {
      lines.push(current);
      current = words[i];
    } else {
      current = current ? current + " " + words[i] : words[i];
    }
  }
  if (current) lines.push(current);

  const totalChars = label.length;
  const fontSize = lines.length === 1 && totalChars <= 2 ? 48
    : lines.length === 1 && totalChars <= 4 ? 28
    : lines.length <= 2 ? 14 : 12;
  const lineHeight = fontSize * 1.35;
  const blockHeight = (lines.length - 1) * lineHeight;
  const startY = 60 + fontSize * 0.35 - blockHeight / 2;

  let textSvg = "";
  for (let i = 0; i < lines.length; i++) {
    const y = startY + i * lineHeight;
    const compress = lines[i].length > maxChars
      ? ' textLength="110" lengthAdjust="spacingAndGlyphs"' : '';
    textSvg +=
      '<text x="60" y="' + y.toFixed(1) + '" text-anchor="middle" font-family="system-ui,sans-serif" ' +
      'font-size="' + fontSize + '" font-weight="600" fill="' + fill + '"' + compress + '>' +
      lines[i] + '</text>';
  }

  return (
    "data:image/svg+xml," +
    encodeURIComponent(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120">' +
        '<rect width="120" height="120" fill="' + bg + '"/>' +
        textSvg +
        '</svg>'
    )
  );
}

function luma(hex) {
  const c = hex.replace("#", "");
  const r = parseInt(c.substring(0, 2), 16) / 255;
  const g = parseInt(c.substring(2, 4), 16) / 255;
  const b = parseInt(c.substring(4, 6), 16) / 255;
  return 0.299 * r + 0.587 * g + 0.114 * b;
}

function escapeXml(s) {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

function processThemeData(raw) {
  const items = raw.items.map(function(item, i) {
    return {
      name: item.name,
      image: item.image === "auto" ? placeholderSvg(item.name, i) : item.image,
    };
  });
  return { id: raw.id, title: raw.title, items: items };
}

function encodeCustomItems(names) {
  const joined = names.join("|");
  const encoded = btoa(unescape(encodeURIComponent(joined)));
  return encoded.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}

function decodeCustomItems(encoded) {
  let base64 = encoded.replace(/-/g, "+").replace(/_/g, "/");
  while (base64.length % 4) base64 += "=";
  return decodeURIComponent(escape(atob(base64))).split("|");
}

function buildCustomTheme(names) {
  const items = names.map(function(name, i) {
    return { name: name, image: placeholderSvg(name, i) };
  });
  return { id: "custom", title: "Custom (Text)", items: items };
}

function nameFromUrl(url) {
  try {
    const path = url.split("?")[0].split("#")[0];
    const filename = path.split("/").pop();
    let name = filename.replace(/\.[^.]+$/, "");
    name = name.replace(/[-_]+/g, " ").trim();
    return name || "item";
  } catch (e) {
    return "item";
  }
}

function encodeUrlItems(items, prefix) {
  const joined = items.map(function(it) {
    const suffix = prefix ? it.image.substring(prefix.length) : it.image;
    return it.name + "\t" + suffix;
  }).join("|");
  const encoded = btoa(unescape(encodeURIComponent(joined)));
  return encoded.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}

function decodeUrlItems(encoded, prefix) {
  let base64 = encoded.replace(/-/g, "+").replace(/_/g, "/");
  while (base64.length % 4) base64 += "=";
  const joined = decodeURIComponent(escape(atob(base64)));
  return joined.split("|").map(function(pair) {
    const parts = pair.split("\t");
    return { name: parts[0], image: (prefix || "") + (parts[1] || "") };
  });
}

function buildUrlScrapeTheme(items) {
  return { id: "urlscrape", title: "Custom (Images)", items: items };
}

function loadTheme(themeId, encodedItems, urlData) {
  if (themeId === "custom") {
    const encoded = encodedItems || getParam("citems") || sessionStorage.getItem("bingo_custom_items");
    if (!encoded) return Promise.reject(new Error("No custom items found"));
    const names = decodeCustomItems(encoded);
    return Promise.resolve(buildCustomTheme(names));
  }

  if (themeId === "urlscrape") {
    const uenc = (urlData && urlData.items) || getParam("uitems") || sessionStorage.getItem("bingo_url_items");
    const upre = (urlData && urlData.prefix) || getParam("uprefix") || sessionStorage.getItem("bingo_url_prefix") || "";
    if (!uenc) return Promise.reject(new Error("No URL scrape items found"));
    const items = decodeUrlItems(uenc, upre);
    return Promise.resolve(buildUrlScrapeTheme(items));
  }

  // Find inline fallback
  let fallback = null;
  for (let i = 0; i < THEMES.length; i++) {
    if (THEMES[i].id === themeId) { fallback = THEMES[i]; break; }
  }

  return fetch("themes/" + themeId + ".json")
    .then(function(res) {
      if (!res.ok) throw new Error("HTTP " + res.status);
      return res.json();
    })
    .then(processThemeData)
    .catch(function() {
      // fetch fails on file:// protocol — use inline fallback
      if (fallback && fallback.items && fallback.items.length > 0) return processThemeData(fallback);
      throw new Error("Theme not found: " + themeId + ". Are you using a local server?");
    });
}
